...
# ------------------------------
# Critical Fix 1: Fix Typo in Initialization Function
# (Previous code had a typo: setup_init_data() instead of safe_init_data())
# ------------------------------
def safe_init_data():
    ...
    # New: Track wheel spin state to prevent infinite loops
    if 'spinning' not in st.session_state:
        st.session_state.spinning = False

# Initialize data correctly (fixed typo)
safe_init_data()
...
def draw_wheel(rotation_angle=0):
    """Draw wheel with proper figure cleanup"""
...
with tab2:
    ...
    # ------------------------------
    # Critical Fix 2: Replace Blocking Animation with Static Result
    # (Long loops with time.sleep() cause Streamlit to hang/cycle)
    # ------------------------------
    if st.session_state.is_admin:
        ...
        with col_wheel:
            ...
            if st.button("Spin Wheel") and not st.session_state.spinning:
                st.session_state.spinning = True  # Prevent duplicate spins
                student = st.session_state.credit_data[st.session_state.credit_data['Name'] == student_name].iloc[0]
                ...
                if student['Total_Credits'] < 50:
                    ...
                    # Critical: Remove blocking loop - show final result directly
                    st.write("Spinning...")
                    time.sleep(1)  # Short delay instead of 50 steps
                    ...
                    st.session_state.spinning = False  # Reset spin state
